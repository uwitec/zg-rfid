...
<html>
	<head>
		...
		<link type="text/css" href="${ctx}/resources/css/${_theme}/frame.css"
			rel="stylesheet" />
		<script src="${ctx}/dwr/interface/ZgTbomCancelDwrAction.js"
...
#matnrCode {
	...
	color: #1d5987;
		...
		function save(){
			...
			if (!checkforstorageCancelInCreate()){
				return;
			}
			var form=document.forms[0];
			form.action="${ctx}/zg/storage/ZgTstorageCancle/save.do";
			form.submit();
		}
		...
		function submitPlan(){	
			...
			var form=document.forms[0];
			form.action="${ctx}/zg/storage/ZgTstorageCancle/submit.do";
			form.submit();
		}
		
		//保存及提交校验，校验数据是否发生变动
		function checkforstorageCancelInCreate(){
			DWREngine.setAsync(false);
			var result = '';
			ZgTbomCancelDwrAction.checkForGenerateCanelCreate(function(data){
				if(data){
					result = true;
				}else{
	...
	<body>
		...
		<div class="eXtremeTable">
				...
				<table class="formitem" width="100%" cellpadding="0" cellspacing="1"
					...
					<tbody id="tbody_1" style="display: block">
						...
						<tr>
							...
							<td width="15%">
								<input type="text" size="25" readonly="true" id="cuid"
									name="cuid" value="${model.cuid }" maxlength="40" />
							</td>
							...
							<td width="15%">
								<input type="text" size="20" readonly="true" id="createDate"
									name="createDate"
						...
						<tr>
							...
							<td colspan="7">
								<input type="text" size="20" maxLength="170"
			...
			<iframe
				src="${ctx}/zg/storage/ZgTstorageCancle/findBomlList.do?id=${model.cuid}&productType=${productType}"  id="listFrame"
