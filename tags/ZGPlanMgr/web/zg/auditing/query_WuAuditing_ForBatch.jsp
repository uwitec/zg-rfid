...
<head>
	...
	<script type="text/javascript">
		...
		function doQuery() {
			...
			if(batchValidation('listFrame','${ctx}/wu/auditing/AuditingForBatch/listForBatch.do',document.forms[0])) {
				document.forms[0].submit();
			}
		}
		
		function orderPlanSubmit() {
			...
			if(flag) {
				form.action = "${ctx}/zg/plan/ZgTorderPlanForBatch/submitOrderPlan.do";
				form.submit();
			}else {
				alert("请选择要提交的计划！");
			}
...
<body>
<div id="_queryPanel">
	<form action="${ctx}/wu/auditing/AuditingForBatch/listForBatch.do" method="post" target="listFrame">
		...
		<table id="query_table" class="query_panel" cellpadding="0" cellspacing="0">
			<thead>
				...
				<input type="text" dateFlag="true" readonly ="true"  value="${ planDate_start}"  name="s_planDate_start" id="s_planDate_start" onchange="checkBeforeAndAfterTime('s_planDate_start','s_planDate_end','beforeTime_oldValue','开始时间','结束时间',1)"/>
					</td>
					<td width="80px" class="label">结束时间：</td>
					<td width="180px">
				<input type="hidden" id="afterTime_oldValue" value="${ planDate_end}"/>
				<input type="text" dateFlag="true" value="${ planDate_end}" name="s_planDate_end" readonly ="true" id="s_planDate_end" onchange="checkBeforeAndAfterTime('s_planDate_start','s_planDate_end','afterTime_oldValue','开始时间','结束时间',2)"/>
					</td>
			...
			<tbody attr="tbody_1" class="querybar unexpand">
				<tr>
					<td width="20px" align="center" rowspan="3" ></td>
						...
						<td><input type="text" size="20" name="s_cuid" maxlength = "40"/></td>
			...
			<tfoot class="querybar">
				<tr>
					<td colspan="7" align="right">
						<div class="toolbar">
							<a id="queryBtn" href="javascript:if(batchValidation('listFrame','${ctx}/wu/auditing/AuditingForBatch/listForBatch.do',document.forms[0]))document.forms[0].submit();"><span><img src="<%=iconPath%>/ico_search.gif" />查询</span></a>
							...
							<a href="javascript:orderPlanSubmit()"><span><img src="<%=iconPath%>/true.gif" />审核已选</span></a>
							<a href="javascript:targetFrame('${ctx }/zg/plan/ZgTorderPlanbomForBatch/findBomBatchList.do')">
									<span><img src="<%=iconPath%>/addition.gif" />新单
									</span>
								</a>
							</c:if>
