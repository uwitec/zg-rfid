...
<div id="infoPanel">
	...
	<iframe id="orderDetailFrame" src="<c:url value="/zg/plan/ZgTorder/show.do?id=${orderId}&publishNum=${publishNum}"/>" autolayout="true" width="100%" frameborder="0" scrolling="no"></iframe>
</div>
<div id="dataPanel">
			...
			<table id="bomTable" cellspacing="0" cellpadding="0">
				<thead>
					...
					<td class="tableHeader" style="color:red"></td>
						...
						<td class="tableHeader">已领取数量</td>
						<td class="tableHeader">备料库存数量</td>
						<td class="tableHeader">移单数量</td>
						<td class="tableHeader">已退料数量</td>
						<td class="tableHeader">已出库数量</td>
					</tr>
				</thead>
				<tbody>
					<tr>
				...
				<c:forEach items="${orderboms}" var="orderbom">
					<c:if test="${orderbom.key.idnrk ne matnr}">
						<tr class="${trcss}">
							...
							<td align="center"></td>
							<td align="center">${orderbom.key.semifinStoreNum}</td>
							<td align="center"></td>
							<td align="center"></td>
							<td align="center"></td>
						</tr>
				   ...
				   <c:forEach items="${orderbom.value}" var="obj" varStatus="n">
					...
					<c:choose>
						...
						<td align="center">${obj.remainNum}</td>
						<td align="center">${obj.moveNum}</td>
						<td align="center">${obj.backNum}</td>
						<td align="center">${obj.outNum}</td>
					</tr>
					</c:forEach>
				</c:forEach>
				</tbody>
