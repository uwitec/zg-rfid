...
//		zgTcarplanBo.update(zgTcarplan);
	...
	public List<Map> getBomLIstByUserId(String planType, String operatorId,String lgort) {
		...
		sql.append("     bom.carnum carCount,orderbom.zbz,carbom.storage_user_id, od.aufnr,  od.arbpl,     od.kdauf,     od.kdpos,  od.maktx1,planbom.bom_type,planbom.zbz pbzbz ");
	 ...
	 * @param operatorInfo
	 */
	public void generateCarPlan(String[] items,List<ZgTcarbomEx> carbomList,OperatorInfo operatorInfo,String planType,Date creatDate) {
			int num=0;
	 ...
	 */
	public String  confirmCarPlan(String[] items,List<ZgTcarbomEx> carbomList,OperatorInfo operatorInfo,String storageUserId,String planType,String remark,Date createDate) {
			...
			for(ZgTcarbomEx bom:carbomList){
				...
				if(num==0){
					...
					if(null==zgTcarplan){//装车计划还没有生成，生成装车计划
						...
						zgTcarplan.setCuid(bom.getCarPlanId());
						zgTcarplan.setCreateUserId(operatorInfo.getOperatorId());
						zgTcarplan.setCreateDate(createDate);
						zgTcarplan.setCarDate(Calendar.getInstance().getTime());
						zgTcarplan.setCarState(Constants.CarPlanStatus.NEW.value());
						zgTcarplan.setOrderPlanType(planType);
						zgTcarplan.setType(Constants.CarPlanType.STOREGETDATA.value());
