...
public class ZgTBomManagerAction extends BaseStruts2Action implements Preparable,ModelDriven{
   ...
   /*
	...
	public String deleteBom(){
		...
		PageRequest<Map> pageRequest = newPageRequest(DEFAULT_SORT_COLUMNS);
		getRequest().setAttribute("planType", IbatisDAOHelper.getStringValue(pageRequest.getFilters(), "planType"));
...
//					break;
			...
			if(flag){//原来的session中没有该bom 则新增进去
				...
				map.put("isDel",false);
				map.put("isModity", true);
				map.put("BOM_TYPE", Constants.QUALITY);
				bol.add(map);
				
				if(!planType.equals(Constants.OrderPlanType.RENEW.value())){
					map=new HashMap();
					map.put("CUID", (java.lang.String)params.get("orderBomId"));
					...
					map.put("isDel",false);
					map.put("isModity", true);
					map.put("BOM_TYPE", Constants.NOTQUALITY);
					bol.add(map);
				}
...
//		ZgTorder zgTorder=zgTorderBo.getById(zgTorderPlan.getOrderTaskId());
	 ...
	 * @throws IOException 
	 */
	public void manulFinishBom() throws IOException{
		for(int i = 0; i < items.length; i++) {
			Hashtable params = HttpUtils.parseQueryString(items[i]);
			String pbId=(String)params.get("pbId");
			zgTBomManagerBo.manulFinishBom(pbId);
		}
		rendHtml("parent.document.forms[0].submit();");
	}
