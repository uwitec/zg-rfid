...
$.widget( "ui.autocomplete_ex", {
	...
	_create: function() {
		...
		this.element
			...
			.bind( "dblclick.autocomplete", function(event) {
				clearTimeout( self.searching );
				self.search( self.element.val()+"%%", event );
			})
			.bind( "keydown.autocomplete",function(event){
				if(event.keyCode == 8) {
					self.element.next().val("");
				}
			})
			.bind( "blur.autocomplete", function( event ) {
				clearTimeout( self.searching );
				if(self.element.next().val() == "") {
					self.element.val("");
				}
