...
<head>
	...
	<script type="text/javascript">
		...
		function view(orderId,publishNum) {
			if(parent.targetFrame) {
				//parent.targetFrame('${ctx}/zg/plan/ZgTorderbom/findViewBomDE.do?orderId='+orderId);
				parent.targetFrame('${ctx}/zg/plan/ZgTorderbom/orderProdConQuery.do?orderId='+orderId+'&publishNum='+publishNum);
			}
		}
		function doLayout() {
...
<body>
<form id="ec" action="${ctx}/zg/plan/ZgTorder/list5.do" method="post" style="display: inline;">
	<div>
		<div>
		...
		<div id="grid-panel" class="grid-panel">
			...
			<div id="grid-data-panel" class="grid-data-panel">
				<table cellspacing="0" cellpadding="0">
					<thead>
						<tr>
							...
							<td class="tableHeader">成品已下线数量</td>
				...
				<!-- 		<td class="tableHeader" onclick="queryColumn(this,'t0_ORDER_STATE')" title="排序 单据状态" >单据状态<%=map.get("ec_image_t0_ORDER_STATE")==null?"":map.get("ec_image_t0_ORDER_STATE")%></td>  	
					...
					<c:forEach items="${page.result}" var="obj" varStatus="n">
						...
						<tr class="${trcss}" title="双击查看" ondblclick="view('${obj.cuid}','${obj.publish_num}')">
							</c:when>
						...
						<tr class="${trcss}" title="双击修改" ondblclick="edit('${obj.cuid}','${obj.pflag}')">
							...
							<td align="center">${obj.publish_num}</td>
							<td>${obj.matnr}</td>
							<!-- 
							<td>
							<c:if test="${obj.pflag=='X'}">已排序</c:if>
							<c:if test="${empty obj.pflag}">未排序</c:if>
							</td> -->
						</tr>
					</c:forEach>
						<tr>
