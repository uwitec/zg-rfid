...
<div id="block" style="display:none">
						...
						<c:forEach items="${bomList}" var="obj" varStatus="n" >
							...
							<div id="${num1}_bom_print">
								...
								退料编号:${pageRequest.filters.carPlanId }
								订单号:${obj.AUFNR}<br/>
								生产线：${obj.ARBPL}<br />
								...
								物料描述:${obj.MAKTX2}<br /> 
								大小量纲:${obj.ZBZ} <br />
									退料数量:<span id="carPlaNum${num1}">${obj.CAR_PLAN_NUM}</span>
								 <br /> <br /> </font>
								 </span>
								 </div>
						    </div>
					...
					<thead>
						<tr>
							<td class="tableHeader" width="3%">
							 ...
							 <td class="tableHeader">需退料数 </td>
							 <td class="tableHeader">本次退料数量 </td>
						</tr>
					</thead>
					<tbody>
					...
					<c:forEach items="${bomList}" var="obj" varStatus="n">
						...
						<tr class="${trcss}">
							...
							<c:if test="${empty obj.STORAGE_USER_ID}">
								...
								<input type="text" maxValue="${obj["WAIT_BACK_NUM"]}" oldValue="${obj.CAR_PLAN_NUM}" maxlength="13" onchange="checkPlanNum(this,'${num}')" size="8" name="carbomList[${num}].planNum" id="carbomList[${num}].planNum" value="${obj.CAR_PLAN_NUM}"/>
						...
						<c:forEach items="${obj.subList}" var="sup" varStatus="v">
							...
							<td  colSpan="3"  align="left">&nbsp;&nbsp;供应商：${sup.lifnrName}</td>
							<td align="center" colSpan="3"> 批次:
								<c:if test="${empty obj.STORAGE_USER_ID}">
