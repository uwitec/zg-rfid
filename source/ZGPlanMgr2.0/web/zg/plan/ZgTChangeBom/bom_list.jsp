	...
	<head>
		...
		function checkWait_back_num(obj,wait_back_num,qualityObjId,notQualityObjId){
			...
			if(isNumber(value)) {
				...
				if(max < qualityCou*1+notQualityCou*1) {
					if("${planType}"!="RENEW"){
						alert("该物料良品不良品总数量必须小于最大退换料数量！");
										...
										<td>
										    <c:choose>
								            	...
								            	<c:if test="${planType=='CHANGE'}">
								            		<input type="text" name="bomList.waitBackNum" value="${obj.WAIT_BACK_NUM}" id="WAIT_BACK_NUM_${obj.BOM_TYPE}_${obj.CUID}" maxValue="${obj.MAX_WAITBACKNUM}" maxLength="13" size="8" 	onchange="checkWait_back_num(this,'${obj.WAIT_BACK_NUM}','WAIT_BACK_NUM_1_${obj.CUID}','WAIT_BACK_NUM_-1_${obj.CUID}')" />
												...
												<c:if test="${fn:indexOf(planType,'BACK')>=0}">
													<input type="text" name="bomList.waitBackNum" value="${obj.WAIT_BACK_NUM}" id="WAIT_BACK_NUM_${obj.BOM_TYPE}_${obj.CUID}" maxValue="${obj.MAX_WAITBACKNUM}" maxLength="13" size="8" 	onchange="checkWait_back_num(this,'${obj.WAIT_BACK_NUM}','WAIT_BACK_NUM_1_${obj.CUID}','WAIT_BACK_NUM_-1_${obj.CUID}')" />
													<input type="hidden" name="bomList.cuid" value="${obj.CUID}" />
													<input type="hidden" name="bomList.bomType" value="${obj.BOM_TYPE}" />
													<input type="hidden" name="bomList.idnrk" value="${obj.IDNRK}" />
												...
												<c:if test="${planType=='RENEW'}">
													<input type="text" name="bomList.waitBackNum" value="${obj.CAR_NUM}" id="WAIT_BACK_NUM_${num}"  maxLength="13" size="8" 	onchange="checkWait_back_num(this,'${obj.CAR_NUM}','WAIT_BACK_NUM_1_${obj.CUID}','WAIT_BACK_NUM_-1_${obj.CUID}')" />
													<input type="hidden" name="bomList.cuid" value="${obj.CUID}" />
													<input type="hidden" name="bomList.bomType" value="${obj.BOM_TYPE}" />
													<input type="hidden" name="bomList.idnrk" value="${obj.IDNRK}" />
												</c:if>
