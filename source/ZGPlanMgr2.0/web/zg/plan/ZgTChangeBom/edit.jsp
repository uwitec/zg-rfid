...
#matnrCode {
	...
	color: #1d5987;
		...
		//	}
			...
			var waitBackNumObjs=document.frames('listFrame').document.getElementsByName("bomList.waitBackNum");
			var cuidObjs=document.frames('listFrame').document.getElementsByName("bomList.cuid");
			var idnrkObjs=document.frames('listFrame').document.getElementsByName("bomList.idnrk");
			var preCuid="";
			for(var i=0;i<waitBackNumObjs.length;i++){
				if(waitBackNumObjs[i].value==0){
					if(cuidObjs[i].value==preCuid){
						alert(idnrkObjs[i].value+' 良品不良品总换料数量为0，请更改');
						butsetDisable(false);
						return;
					}else{
						preCuid=cuidObjs[i].value;
					}
				}
					...
					<tr>
						<td class="title" colspan="10">
							<img src="${ctx }/resources/images/frame/ico_noexpand.gif"
								...
								<c:if test="${fn:indexOf(model.planType,'BACK')>=0}">退料申请单</c:if>
