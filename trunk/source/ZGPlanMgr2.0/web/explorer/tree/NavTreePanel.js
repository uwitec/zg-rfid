$importjs(ctx+"/dwr/interface/ExplorerTreeAction.js");
...
DRM.NTTreePanel = function(config) {
	DRM.NTTreePanel.superclass.constructor.call(this, Ext.apply(config, {
		...
		clickType	: typeof(config.clickType) == 'undefined'? 'click' : config.clickType,
		checkIds	: typeof(config.checkIds) == 'undefined'? 'click' : config.checkIds,
		isExpandAll : typeof(config.isExpandAll) == 'undefined'? false : config.isExpandAll
	}));
};
...
Ext.extend(DRM.NTTreePanel, Ext.tree.TreePanel, {
	...
	initComponent : function() {
		...
		this.on(this.clickType, function(node, event) {
			var nodeAttributes = node.attributes;
			if (nodeAttributes != null && typeof nodeAttributes != "undefined") {
				var bmClassId = nodeAttributes.bmClassId;
				var cuid = nodeAttributes.cuid;
		...
		if(this.isInit){
			this.initTree(this.templateId, this.treeObject,this.isExpandAll);
		};
		//if(this.showChecked){
			//this.addButton({text: '确定'},this._getCheckedNodes,this);
		//};
	},
