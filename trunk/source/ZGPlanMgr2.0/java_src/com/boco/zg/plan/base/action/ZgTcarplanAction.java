...
public class ZgTcarplanAction extends BaseStruts2Action implements Preparable,ModelDriven{
	...
	//	String plant=getSession().getAttribute("defaultPlant")==null?"":getSession().getAttribute("defaultPlant").toString();
		...
		getRequest().setAttribute("pageRequest", pageRequest);
		if((pageRequest.getFilters().get("planType").toString()).contains("BACK")){
			return LIST_JSP1;
		}else {
			return LIST_JSP;
		}
...
//		return LIST_ACTION;
	...
	public void generateCarPlan() throws IOException{
		...
		Date createDate=TimeFormatHelper.convertDate(pageRequest.getFilters().get("createDate").toString(),TimeFormatHelper.TIME_FORMAT_A);
		zgTcarplanExBo.generateCarPlan(items,carbomList,operatorInfo,planType,createDate);
	...
	public void confirmCarPlan() throws IOException{
		...
		Date createDate=TimeFormatHelper.convertDate(pageRequest.getFilters().get("createDate").toString(),TimeFormatHelper.TIME_FORMAT_A);
		String result=zgTcarplanExBo.confirmCarPlan(items,carbomList,operatorInfo,storageUserId,planType,remark,createDate);
...
//		getRequest().setAttribute("carPlanId", carPlanId);
	...
	public String loadCar() {
		...
		getRequest().setAttribute("advance", advance);
		getRequest().setAttribute("groupId", groupId);
		getRequest().setAttribute("createDate", TimeFormatHelper.getFormatDate(Calendar.getInstance().getTime(),TimeFormatHelper.TIME_FORMAT_A));
		...
		if(carPlanList.size()>0){
			...
			getRequest().setAttribute("createDate", carPlan.get("CREATDATE"));
		}else {
...
//				}
		...
		getRequest().setAttribute("bomList", bomList);
		pageRequest.getFilters().put("bomCuids", bomCuids);
		getRequest().setAttribute("createDate", pageRequest.getFilters().get("createDate"));
		getRequest().setAttribute("pageRequest", pageRequest);
		if(planType.contains("BACK")){
			return CAR_BOM_LIST1;
		}else {
