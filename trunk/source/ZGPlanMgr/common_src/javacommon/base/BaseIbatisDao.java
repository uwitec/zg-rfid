 ...
 * @version 1.0
 */
public abstract class BaseIbatisDao<E,PK extends Serializable> extends BaseDao implements EntityDao<E,PK> {
    ...
    public String getFindByPropertyQuery() {
    	return getEntityClass().getSimpleName()+".findByProperty";
    }
	...
	/**
	 * 
	 * @param statementName 执行的sql id
	 * @param statementCount  统计条数 sql id
	 * @param pageRequest
	 * @return
	 */
	protected Page pageQuery(String statementName,String statementCount ,PageRequest pageRequest) {
	...
	public List findAll() {
		return getSqlMapClientTemplate().queryForList(getFindAllQuery());
	}
	
	public List findByProperty(E entity) {
		Map parameterObject = new MapAndObject(new HashMap(),entity);
		List list = getSqlMapClientTemplate().queryForList(
				getFindByPropertyQuery(), parameterObject);
		return list;
	}
	...
	public List findByProperty(E entity,String orderColumn,boolean isAsc) {
		Map otherFilters = new HashMap();
		String orderBy = orderColumn;
		if(!isAsc) {
			orderBy += " desc"; 
		}
		otherFilters.put("sortColumns", orderBy);
