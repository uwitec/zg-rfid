...
<head>
	...
	<script type="text/javascript">
		...
		function doQuery() {
			...
			if(batchValidation('listFrame','${ctx}/zg/plan/ZgTorder/list1.do',document.forms[0])) {
		...
		*/
		...
		function finishOrder(){
			...
			if (confirm('确定执行[手工结单]操作?')){
				document.frames('listFrame').document.forms[0].action = "${ctx}/zg/plan/ZgTorder/finishOrder.do";
				document.frames('listFrame').document.forms[0].submit();
			}
		}
		
		function query(){
			var aFinish=document.getElementById("aFinish");
			var state=document.getElementById("s_orderState").value;
			if(state=="10"){
				aFinish.style.display="none";
			}else{
				aFinish.style.display="";
			}
			
		 	if(batchValidation('listFrame','${ctx}/zg/plan/ZgTorder/list1.do',document.forms[0]))document.forms[0].submit();
		}
	</script>
</head>
...
<body>
<div id="_queryPanel">
	<form action="${ctx}/zg/plan/ZgTorder/list1.do" method="post" target="listFrame">
				...
				<tr>
					<td colspan="7" align="right" class="toolbar">
						<div class="toolbar">
							...
							<a id="aFinish" href="javascript:finishOrder()"><span><img src="<%=iconPath%>/true.gif" />手工结单</span></a>
						</div>
					</td>
				</tr>
			</tfoot>
