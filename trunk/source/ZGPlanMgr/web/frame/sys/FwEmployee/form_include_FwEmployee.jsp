...
<!-- ONGL access static field: @package.class@field or @vs@field -->
	...
	<tr>
		...
		<td>
	   		<input type="text" value="${model.userId}" size="30" maxlength="20" name="userId" id="userId" class="required"/>
		</td>
	</tr>
	...
	<tr>
		...
		<td>
			<input type="text" size="30" value="${model.orgId_related.value}" maxlength="40" autocomplete="true" xtype="tree:1" id="orgId_label" columnNameLower="orgId" bmClassId="FW_ORGANIZATION" column="m.t0_LABEL_CN" class="required"/>
	...
	<tr>
		...
		<td>
		<select id="orgIds" name=""orgIds"" style="width:216px" ondblclick="removeOptionFromSelect('orgIds')" size="4"  multiple>
		</select>
			&nbsp;<img src="./resources/images/frame/autocomplete.png" style="cursor: pointer" onclick="selectManageOrg()"/>
			...
			<br/>
					...
					<c:choose>
							<c:when test="${fwOperatorRoleMap[role.cuid] eq role.cuid}">
								<input type="checkBox" checked="true" name="roles" value="${role.cuid }"/>${role.labelCn}
							</c:when>
							<c:otherwise>
								<input type="checkBox" name="roles" value="${role.cuid }"/>${role.labelCn}
							</c:otherwise>
					</c:choose>
